                ...
                <div class="panel-body">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-xs-3 col-sm-2 col-md-1 postal-code-line" id="first_postalcode">
                            <input type="text" class="form-control"  name="first_postalcode" value="<?php  if( isset($meta['zip']) && strpos($meta['zip'], '-')) { $postalcode = explode('-',$meta['zip']) ; echo $postalcode[0];  } ?>" maxlength = "4" />
                        </div>
                        <div class="col-xs-3 col-sm-2 col-md-1" id="second_postalcode">
                            <input type="text" class="form-control"  name="second_postalcode" value="<?php  if( isset($meta['zip']) && strpos($meta['zip'], '-')) { $postalcode = explode('-',$meta['zip']) ; echo $postalcode[1];  } ?>" maxlength = "4" />
                        </div>
                        <div class="col-xs-3 ">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <input type="tel" class="form-control" value="<?php echo isset( $meta['tel'] )? $meta['tel'] : '' ; ?>" name="student_phone" maxlength = "11">
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-5">
                            <input type="tel" class="form-control" value="<?php  echo isset( $meta['emergency_tel'] )? $meta['emergency_tel'] :'' ?>" name="student_emergency_tel" maxlength = "11" >
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" name="course_main" style="padding-left:25em">
                                ...
                                if( $course['all'] )
                                {
                                    $currentcourse_id =  $course['valid'][0]['course_id'];
                                    foreach($course['all'] as $item)
                                    {
                                        $selected = '';
                                        if($item['course_id'] === $currentcourse_id)
                                            $selected = 'selected';
                                        echo ('<option data-pratice = "'.$item['practice_max'].'" value="'.$item['course_id'].'" '.$selected.' >'.$item['course_code'].' '.$item['course_name'].'</option>');
                                    }
                                }
                                else{
                                            ...
                                            elseif( $key != 0 ){
                                                ...
                                                foreach ( $arr_class_join as $subkey_1 => $subvalue_1 ) {
                                                    ...
                                                    if( $value == $subvalue_1['base_class_sign'] && $subvalue_1['week_num'] == strval($x)  )
                                                    {
                                                        ...
                                                        $join =  intval( $subvalue_1['week_full'][1] ) - 1;
                                                        $choose = '選択';
                                                ...
                                                echo '<td '.$data_class. $data_base.$data_id.$data_week_full. ' onclick="choose_class(this)" >'.$choose.'</td>';
                                            }
                                        }
                                        echo '</tr>';
                                    }
                                    ...
                                    <div class="col-sm-6">
                                        ...
                                        <div class="row">
                                            <div class="col-xs-6">
                                                ...
                                                $year_leave_club = isset($meta['enquete']) && isset( json_decode($meta['enquete'])->experience->club_name ) && json_decode($meta['enquete'])->experience->year != "" ? json_decode($meta['enquete'])->experience->year : date_format (date_create (START_DATE_DEFAULT) , 'Y');
                                                $month_leave_club = isset($meta['enquete']) && isset( json_decode($meta['enquete'])->experience->club_name) && json_decode($meta['enquete'])->experience->month!= "" ? json_decode($meta['enquete'])->experience->month : date_format (date_create (START_DATE_DEFAULT) , 'm');
                        ...
                        <div class="col-sm-10 col-sm-offset-2 select_bus_route"  id="select_bus_route">
                            ...
                            if( count( $bus_course['all'] ) > 0){
                                ...
                                foreach ( $bus_course['all'] as $key => $value) {
                                    if( isset( $value['student_join']) ) ++$index_join ;
                                }

                                foreach ( $bus_course['all'] as $key => $value) {

                                    $class = $value['available'][0]['class_name'];
                                    $base_class_sign =  $value['available'][0]['base_class_sign'];
                                    $class_id =  $value['available'][0]['class_id'];
                                    $config = $this->configVar;
                                    $week_num = $config['week_num'];
                                    ...
                                    if( isset( $value['student_join'][0] ) )
                                    {
                                        ...
                                        $html = array();
                                        $html[] = '<div class="element_bus_course" data-sign="'.$base_class_sign.'-'.$week.'" data-id="'.$class_id.'"><div for="" class"col-sm-2 control-label " id="classnameforbus">'.$week_num[$week].'<br>('.$class.')</div>';
                                ...
                                <select class="form-control bus_course" name="bus_course_go_'.$index.'" '.''.$disable .' onchange="changeBuscoure(this)">';
                                        $html[] = '';
                                        $html[] = '</select></div>';
                                        $html[] = '<div class="col-sm-5"><select class="form-control bus_stop" name="bus_stop_go_'.$index.'" '.$disable.'>';
                                        $html[] = '';
                                        $html[] = '</select></div></div>';
                                ...
                                <select class="form-control bus_course" name="bus_course_ret_'.$index.'" '.''.$disable .' onchange="changeBuscoure(this)">';
                                        $html[] = '';
                                        $html[] = '</select></div>';
                                        $html[] = '<div class="col-sm-5"><select class="form-control bus_stop" name="bus_stop_ret_'.$index.'" '.$disable.'>';
                                        $html[] ='';
                                        ...
                                        foreach ( $value['available'] as $subkey => $subvalue ) {
                                            $selected_go = '';
                                            $selected_ret = '';
                                            $selected_stop_go ='';
                                            $selected_stop_ret = '';
                                        ...
                                        if( $index_bus == 0 &&  $index_join > 1)
                                        {
                                            $set_same  = '<div class="checkbox ml-1" id="set_same" ><label id="set_same_lable" style="margin: 0px auto 10px 142px;padding-top: 0px;" onclick ="check_set_same()"><input type="checkbox" id="check_set_same">';
                    ...
                    <!-- <div class="form-group row">
                        <label class= "col-sm-2  control-label" >入会金区分</label>
                        <div class = "col-sm-3 offset-sm-7">
                            <select class="form-control" name="item">
                                <?php   
                                    echo '<option value="'.isset( $course['valid'][0]['course_id'] ) ? $course['valid'][0]['course_id'] : $course['all'][0]['course_id'] .'">'
                                    .$item['cost_fee'][0]['sell_price']
                                    .'</option>';
                                ?> 
                            </select>   
                        </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        $(function () {
            var start_date =  $('input[name = rest_start_date]').val().replace(/\s/g, '');
            start_date = new Date(start_date);
            start_date.setDate(start_date.getDate() + 1);
            var end_date = $('input[name = rest_end_date]').val().replace(/\s/g, '') ;
            end_date = new Date(end_date);
            end_date.setDate(end_date.getDate() - 1);
            $('input[name=rest_end_date]').data('datepicker').setStartDate(start_date);
            $('input[name=rest_start_date]').data('datepicker').setEndDate(end_date);
        });

        $('input[name=rest_end_date]').datepicker(options_Y_M_D).on('changeDate', function(){
            var end_date =  $(this).val().replace(/\s/g, '');
            end_date = new Date(end_date);
            end_date.setDate(end_date.getDate() - 1);
            $('input[name=rest_start_date]').data('datepicker').setEndDate(end_date);
        });
        $('input[name=rest_start_date]').datepicker(options_Y_M_D).on('changeDate', function(){
            var start_date = $(this).val().replace(/\s/g, '');
            start_date = new Date(start_date);
            start_date.setDate(start_date.getDate() + 1);
            $('input[name=rest_end_date]').data('datepicker').setStartDate(start_date);
        });
        ...
        $('input[name=student_birthday] ').change(function(){
