...
class Member_model extends DB_Model {
    ...
    public function check_valid_tag($array_data)
    {
        ...
        foreach ($array_data as $key => $value) {
            ...
            ($key == 'class_from')||($key == 'level')||($key == 'practice_course')){              
            }else{
                if($value != ''){
    ...
    public function check_valid_condition2($array_data)
    {
        $status = FALSE;
        foreach ($array_data as $key => $value) {
            if(($key == 'class_from')||($key == 'level')||($key == 'practice_course')){
                if($value != ''){
                    $status = TRUE;//input tag có ít nhất một trường có giá trị
                    break;
                }
    ...
    {   
        ...
        foreach ($array_input as $key => $value) {
            if(($key == 'id_from')||($key == 'all_user')||($key == 'id_to')||($key == 'sub_user')||($key == 'class_from')||($key == 'level')||($key == 'practice_course')){
    ...
    public function filter_list_member($limit, $start, $type)
    {
        ...
        if($check_info_course){
            ...
            $level = $data_input_condition['level'];
            if($level != ''){
                $this->db->like('m_class.base_class_sign', $level);
            }
            
            $class_name = $data_input_condition['class_from'];
            if($class_name != ''){
                $this->db->like('m_class.class_name', $class_name);
            }
        ...
        if(isset($_POST['text_condition'])){
            ...
            switch ($type_search) {
                ...
                case 'practice_course':
                    $this->db->like('m_course.course_code', $input_search);
                    break;
                case 'level':
            ...
            return array('0'=>$data_search_rel,'1'=>$total);
